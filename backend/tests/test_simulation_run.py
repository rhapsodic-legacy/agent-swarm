...
def _setup():
    ...
    stub = Terrain(
        width=10240, height=10240, max_elevation=200.0,
...
def test_simulation_run():
    ...
    for d in world.drones:
        print(f"  Drone {d.id:>2d}: ({d.position.x:7.0f}, {d.position.z:7.0f}) "
    ...
    dist_to_nearest = math.sqrt(
        (nearest_survivor.position.x - base.x) ** 2 +
        (nearest_survivor.position.z - base.z) ** 2,
    )
    ...
    for d in world.drones:
        commands.append(Command(
            type="move_to",
            drone_id=d.id,
            target=Vec3(nearest_survivor.position.x + d.id * 50,
                        0,
    ...
    for i in range(200):
        ...
        discovered_this_tick = [e for e in world.events
                                if e.type.name == "SURVIVOR_FOUND"]
        if discovered_this_tick:
            for e in discovered_this_tick:
                total_found += 1
                print(f"  [tick {world.tick:>3d}] SURVIVOR FOUND! "
    ...
    for d in world.drones:
        dist_from_base = math.sqrt(
            (d.position.x - base.x) ** 2 + (d.position.z - base.z) ** 2
        )
        print(f"  Drone {d.id:>2d}: ({d.position.x:7.0f}, {d.position.z:7.0f}) "
    ...
    max_dist = max(
        math.sqrt((d.position.x - base.x) ** 2 + (d.position.z - base.z) ** 2)
        for d in world.drones
    )
...
def test_drones_discover_survivors():
    ...
    print(f"Nearest survivor: ({nearest.position.x:.0f}, {nearest.position.z:.0f}) dist={dist:.0f}m")
    ...
    commands = [
        Command(
            type="move_to",
            drone_id=d.id,
            target=Vec3(nearest.position.x + (d.id % 3) * 10, 0, nearest.position.z + (d.id // 3) * 10),
        )
        for d in world.drones
    ]
    ...
    for i in range(ticks_needed):
        ...
        for e in world.events:
            if e.type.name == "SURVIVOR_FOUND":
                ...
                if drone and surv:
                    ...
                    print(f"  [tick {world.tick:>4d}] FOUND survivor {e.survivor_id} "
        ...
        if i % 500 == 499:
            ...
            found = sum(1 for s in world.survivors if s.discovered)
            print(f"  [tick {world.tick:>4d}] Drone 0 dist to target: {d2target:.0f}m | "
                  f"found: {found}")
